...
sim_MA_series_chpts = function( n , chpts , eps , rho ){
  
  if (rho != 0){
    data <- arima.sim( n=n , list(ma = rho ))
  }
  else{
    data = rnorm(n)
  }
...
simulate.MA.data = function( n=500 , N=100 , K=2 , eps=5 , rho ){
  ...
  for (i in 1:N){
    ...
    data[i,] = sim_MA_series_chpts( n , chpts.each.series[[i]] , eps , rho )
